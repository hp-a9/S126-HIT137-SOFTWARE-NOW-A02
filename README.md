...
** question2.py (Evaluator / Main Controller) **
 --> Reads expressions from the input file.
 --> Calls tokenizer (q2.py) and parser (parser_refactored.py). These 2 files can be found in branch "Tokenizer"
Formats:
  .Tokens
  .Parse tree
  .Final result
 ...
 --> Writes output to "output.txt" in the required format.
